...
#define BLE_MAX_LLCP_SIZE               (27)
#define BLE_MAX_MESSAGE_SIZE            (256)
#define BLE_HEAP_MGT_SIZE               (4)
#define BLE_LLM_UTIL_CNX_BDADDR_SIZE    (16) //Define insteadof sizeof(struct llm_util_cnx_bd_addr_tag) to prevent including too much files
#define BLE_ADV_REPORT_MAX              (BLE_RX_BUFFER_CNT) // Max ADV REPORT in the same event
#define BLE_LLCP_PDU_SIZE               (16)
#define BLE_ADV_DEV_LIST_SIZE           (12)
#define BLE_SCAN_PDU_PARAMS_SIZE        (12)
#define BLE_ADV_PDU_PARAMS_SIZE         (32)
#define BLE_EA_ELT_SIZE                 (44)
 ...
 */
    ...
    #define BLE_HEAP_MSG_SIZE               ((BLE_CONNECTION_MAX+1) * (BLE_MAX_MESSAGE_SIZE + BLE_MAX_LLCP_SIZE) + BLE_HEAP_MGT_SIZE)
    
    /// Size required to allocate environment variable for a number of links defined by the BLE_CONNECTION_MAX macro
    #define BLE_HEAP_ENV_SIZE               (((sizeof(struct llc_env_tag)+ BLE_HEAP_MGT_SIZE) * (BLE_CONNECTION_MAX))\
            + ((BLE_LLCP_PDU_SIZE + BLE_HEAP_MGT_SIZE) * (BLE_CONNECTION_MAX + 1))\
            + ((sizeof(struct lld_evt_deferred_tag)+ BLE_HEAP_MGT_SIZE) * (BLE_CONNECTION_MAX + 1))\
            + ((sizeof(struct lld_evt_wait_tag)+ BLE_HEAP_MGT_SIZE) * (BLE_CONNECTION_MAX + 1))\
            + ((BLE_ADV_DEV_LIST_SIZE + BLE_HEAP_MGT_SIZE) * 4)\
            + ((BLE_LLM_UTIL_CNX_BDADDR_SIZE+ BLE_HEAP_MGT_SIZE) * BLE_CONNECTION_MAX)\
            + (BLE_SCAN_PDU_PARAMS_SIZE + BLE_HEAP_MGT_SIZE)\
            + (BLE_ADV_PDU_PARAMS_SIZE + BLE_HEAP_MGT_SIZE)\
            + (((sizeof(struct lld_evt_tag) + BLE_EA_ELT_SIZE)+ BLE_HEAP_MGT_SIZE) * (BLE_CONNECTION_MAX+1)))

    #define BLE_TAG_HEAP_ENV_SIZE           (sizeof(struct llc_env_tag*) * BLE_CONNECTION_MAX + 4)
    #define BLE_TX_NODE_HEAP_ENV_SIZE       (((sizeof(struct em_desc_node) * (BLE_CONNECTION_MAX + BLE_TX_DESC_ADV + (CFG_BLE_TX_BUFF_DATA_CNT * 10)) + (sizeof(struct em_buf_node) * CFG_BLE_TX_BUFF_DATA_CNT))))
    #define BLE_KE_HEAP_ENV_SIZE            (sizeof(struct ke_task_elem) * BLE_CONNECTION_MAX + 4)
#else
