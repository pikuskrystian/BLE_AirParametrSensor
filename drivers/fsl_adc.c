...
#define ADC_BANDGAP_DEFAULT ((float)(1221.7))
#define ADC_BANDGAP_MIN     ((float)(1214.0))
#define ADC_BANDGAP_MAX     ((float)(1228.0))

#define ADC_BANDGAP_INFO_ADDR (FSL_FEATURE_FLASH_ADDR_OF_BANDGAP_VOL)

/* This value can be adjusted appropriately according to the actual application scenario */
#define ADC_TEMP_CORRECTION_FACTOR  2
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
/*!
...
void ADC_SetSdConfig(ADC_Type *base, uint8_t idx, const adc_sd_config_t *config)
{
    ...
    mask = ADC_CFG_PGA_GAIN_MASK | ADC_CFG_PGA_BP_MASK | ADC_CFG_PGA_VINN_MASK | ADC_CFG_ADC_GAIN_MASK |
           ...
           ADC_CFG_PGA_VCM_EN_MASK | ADC_CFG_PGA_VCM_DIR_MASK | ADC_CFG_PGA_VCM_MASK | ADC_CFG_ADC_VCM_MASK;
...
{
    float cal = *(uint32_t *)ADC_BANDGAP_INFO_ADDR;
    
    if ((cal < ADC_BANDGAP_MIN) || (cal > ADC_BANDGAP_MAX))
    {
        cal = ADC_BANDGAP_DEFAULT;
    }
...
void ADC_GetTemperatureCalibrationResult(float *coe, float *offset, uint8_t *amb)
{
    uint32_t cal;

    cal = *(uint32_t *)FSL_FEATURE_FLASH_ADDR_OF_TEMP_CAL;

    if((cal >> 24) == 0xAA)                                             /*BIT24 - BIT31*/
    {
        *offset = (cal & 0x3FF);                                        /*BIT0  - BIT9*/
        *coe    = (float)((cal >> 10) & 0x3F) / 100 + 1;                /*BIT10 - BIT15*/
        *amb    = ((cal >> 16) & 0xFF) - ADC_TEMP_CORRECTION_FACTOR;    /*BIT16 - BIT23*/
    }
    else
    {
        *offset = 831.985696;
