...
/*
 * \brief indicates if software timers servicing is pending
 */
static bool_t bTimerServPending;
...
{
    return ((!numberOfActiveTimers) && (!bTimerServPending));
}
...
#else
        useconds = (prescaler1 + prescalerOffset)*(1000000/mCounterFreqHz) + (uint64_t)seconds1*1000000;
#endif
    }
